...
class ChartsApp:
    ...
    def create_chart_window(self, sender, app_data, timeframe):
        ...
        menu_indicators_tag = f'menu_indicators_{self.window_counter}'
        
        self.tags[window_tag] = {
            'plot': plot_tag, 
            'timeframe': timeframe,
            'view' : view_format,
            'x_axis': x_axis_tag,
            ...
            'menu_indicators': menu_indicators_tag, 
            'checked_indicators': list(dict()) 
        }

        with dpg.window(label=timeframe, width=400, height=400, tag=window_tag):
            with dpg.menu_bar():
                with dpg.menu(label="Indicators", tag=menu_indicators_tag):
                    self.update_indicator_menu(window_tag, menu_indicators_tag)
    ...
    def update_indicator_menu(self, window_tag, menu_indicators_tag):
        # Clear existing items in the indicators menu
        children = dpg.get_item_children(menu_indicators_tag)
        if children:
        ...
        for indicator in self.indicators:
            dpg.add_checkbox(label=indicator, callback=self.update_checked_indicators, 
                            user_data=[indicator, window_tag], parent=menu_indicators_tag)
    ...
    def update_checked_indicators(self, sender, app_data, user_data):
        ...
        self.tags[window_tag]['checked_indicators'].append( 
    ...
    def update_stored_indicators(self):
        ...
        ### DONT REMOVE ALL _ JUST GET RID OFF ONE NOT IN AND ADD NEW ONES _ eg this way you dont have to recaluactae indicatprs allready calculauted
        self.indicators = {}
        ...
        if self.tags:
            for window_tag, window_tags in self.tags.items():
                self.update_indicator_menu(window_tag, window_tags['menu_indicators'])
